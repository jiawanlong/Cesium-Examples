export const windData = {
  ...
  BOUNDARY: 0.45,
  fadeToWhite: colorInterpolator(sinebowColor(0.8, 0), [200, 200, 200]),
  ...
  buildGrid: function (builder) {
    ...
    this.interpolate = function (lng_p, lat_p) {
      var i = windData.floorMod(lng_p - lng1, 360) / dx1 // calculate longitude index in wrapped range [0, 360)
      ...
      if ((row = grid[fj])) {
        ...
        var g10 = row[ci]
        if (
          windData.isValue(g00) &&
          windData.isValue(g10) &&
          (row = grid[cj])
        ) {
          var g01 = row[fi]
          var g11 = row[ci]
          if (windData.isValue(g01) && windData.isValue(g11)) {
  ...
  ScreenToLonlat: function (params, pos) {
    ...
    if (resultRay == undefined) {
      ...
      cartographic.longitude *= windData.RTOD
      cartographic.latitude *= windData.RTOD
      return cartographic
    }
  },
  createField: function (columns, bounds, mask) {
    ...
    function field(x, y) {
      ...
      return (column && column[Math.round(y)]) || windData.NULL_WIND_VECTOR
    }
    ...
    field.isInsideBoundary = function (x, y) {
      return field(x, y) !== windData.NULL_WIND_VECTOR
    }
    ...
    field.randomize = function (o) {
      ...
      var safetyNet = 0
      do {
        x = Math.round(windData.random(bounds.x, bounds.xMax))
        y = Math.round(windData.random(bounds.y, bounds.yMax))
  ...
  extendedSinebowColor: function (i, a) {
    return i <= windData.BOUNDARY
      ? sinebowColor((i / windData.BOUNDARY) * 2, a)
      : windData.fadeToWhite(
          (i - windData.BOUNDARY) / (1 - windData.BOUNDARY),
          a
        )
  },
  ...
  distortion: function (params, lng_p, lat_p, x, y) {
    var hlng = lng_p < 0 ? windData.H : -windData.H
    var hlat = lat_p < 0 ? windData.H : -windData.H
    var plng = windData.LonlatToScreen(params, [lng_p + hlng, lat_p])
    var plat = windData.LonlatToScreen(params, [lng_p, lat_p + hlat])
    ...
    var k = Math.cos((lat_p / 360) * windData.T)
  ...
  distort: function (params, lng_p, lat_p, x, y, scale, wind) {
    var u = wind[0] * scale
    var v = wind[1] * scale
    var d = windData.distortion(params, lng_p, lat_p, x, y)
  ...
  windIntensityColorScale: function (step, maxWind) {
    ...
    for (var j = 85; j <= 255; j += step) {
      result.push(windData.asColorStyle(255, 255, 255, 255.0))
    }
  ...
  reCreate: function (params, builder, offscreen) {
    ...
    var velocityScale = windData.velocityScale_o
    ...
    function interpolateColumn(params, x) {
      var column = []
      ySize = 0
      for (var y = bounds.y; y <= bounds.yMax; y += windData.INTERPOLATEDIV) {
        ySize++
        point.x = x
        point.y = y

        var coord = windData.ScreenToLonlat(params, point)
        var color = windData.TRANSPARENT_BLACK
        var wind = null
        if (coord) {
          ...
          if (isFinite(lng_p)) {
            if (builder) {
              ...
              if (windData.isValue(scalar)) {
                color = windData.extendedSinebowColor(
                  Math.min(scalar, 100) / 100,
                  windData.OVERLAY_ALPHA
                )
              }
            } else {
              return
        ...
        column[y + 1] = column[y] = wind || windData.HOLE_VECTOR
        mask
          .set(x, y, color)
          .set(x + 1, y, color)
          .set(x, y + 1, color)
